...
semantics the author's pipeline learned the hard way — a grace window after
the slot, cooldown between failed attempts, a per-day attempt cap, and job
dependencies. Catch-up after laptop sleep needs no special code: every tick
compares the wall clock with today's run record, so a missed slot fires on
the next tick after wake.
"""
