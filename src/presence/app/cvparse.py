...
YEAR_RANGE_RE = re.compile(r"^\d{4}\s*[-–—]\s*(?:\d{4}|present|now)$", re.I)


def guess_phone(text: str) -> str:
    """The first thing that is really a phone number: 9–15 digits, not a year range."""
    for m in PHONE_RE.finditer(text):
        cand = m.group(0).strip()
        digits = re.sub(r"\D", "", cand)
        if 9 <= len(digits) <= 15 and not YEAR_RANGE_RE.match(cand):
            return cand
    return ""
