...
def test_messaged_changes_no_status_even_on_a_closed_job(tracker):
    from datetime import date

    job, _ = tracker.ingest(LI)
    tracker.record_event(job.id, "applied", date(2026, 8, 13))
    tracker.record_event(job.id, "messaged", date(2026, 8, 23))
    assert tracker.get(job.id).status == "applied"
    tracker.record_event(job.id, "rejected", date(2026, 8, 26))
    tracker.record_event(job.id, "messaged", date(2026, 8, 30))  # a note-like kind is still allowed
    assert tracker.get(job.id).status == "rejected"
    assert tracker.timeline(job.id) == (
        "13 Aug → messaged 23 Aug → rejected 26 Aug → messaged 30 Aug"
    )
