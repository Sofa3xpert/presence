...
from presence.app import jd
from presence.connectors import ConnectorError, Posting, boards
...
def test_init_check_and_refusal_before_confirmation(tmp_path, capsys):
    ...
    assert "Confirm your profile first in Setup, step 1" in out
...
def test_cycle_keeps_each_new_job_text_once(tmp_path, monkeypatch):
    cli.main(["init", str(tmp_path)])
    _confirm(tmp_path)
    (tmp_path / "sources.yaml").write_text(
        SOURCES + "  - {id: acme, provider: smartrecruiters, label: Acme, "
        "config: {board: acme}}\n"
    )
    detail = []
    ...
    def fake_get(url, **kw):
        ...
        return {"jobAd": {"sections": {
            "jobDescription": {"title": "Job Description", "text": "<p>Own the numbers.</p>"},
            "qualifications": {"title": "Qualifications", "text": "<ul><li>SQL</li></ul>"},
        }}}

    monkeypatch.setattr(boards, "_get", fake_get)
    postings = [
        Posting(company="Figma", title="Software Engineer, AI", location="London",
                url="https://boards.greenhouse.io/figma/jobs/1", source="greenhouse",
                description="Build & ship."),
        Posting(company="Acme", title="Analyst", location="London",
                url="https://jobs.smartrecruiters.com/acme/5", source="smartrecruiters",
                description_ref="5"),
        Posting(company="Palantir", title="Deployment Strategist", location="London",
                url="https://jobs.lever.co/palantir/9", source="lever"),  # board sent no text
    ]
    monkeypatch.setattr(cycle, "run_sources", lambda *a, **k: (list(postings), {}))
    jd.save(tmp_path, 1, "my own pasted text")  # the first job's text, already on file
    brief, errors, _ = cycle.run_cycle(tmp_path)
    assert "3 new roles" in brief and errors == {}
    assert jd.load(tmp_path, 1) == "my own pasted text"  # never replaced
    assert jd.load(tmp_path, 2) == "Job Description\nOwn the numbers.\n\nQualifications\n- SQL"
    assert not jd.has(tmp_path, 3) and not jd.path(tmp_path, 3).exists()
    assert detail == ["https://api.smartrecruiters.com/v1/companies/acme/postings/5"]
    # the same postings again: nothing new, so no text is fetched or written
    brief, errors, _ = cycle.run_cycle(tmp_path)
    assert "No new roles today" in brief and len(detail) == 1
    assert sorted(p.name for p in (tmp_path / "jd").iterdir()) == ["1.txt", "2.txt"]
    # a board that will not give the text: the job is still tracked, the brief says so
    postings[:] = [Posting(company="Acme", title="Data Analyst", location="Leeds",
                           url="https://jobs.smartrecruiters.com/acme/6",
                           source="smartrecruiters", description_ref="6")]

    def broken(url, **kw):
        raise ConnectorError("HTTPError: 503")

    monkeypatch.setattr(boards, "_get", broken)
    brief, errors, _ = cycle.run_cycle(tmp_path)
    assert "1 new role" in brief and errors == {"Acme": "job text not fetched: HTTPError: 503"}
    assert "Sources that failed: Acme" in brief and not jd.has(tmp_path, 4)
