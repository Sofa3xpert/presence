...
def test_status_and_engine_routes(tmp_path, monkeypatch, darwin):
    ...
    # a real Ollama on this machine must not leak into the test
    monkeypatch.setattr(ollama.requests, "get", lambda *a, **k: (_ for _ in ()).throw(OSError()))
    ...
    assert j["running"] is False and j["recommended"] == "nemotron-3-nano:4b"
